...
const reNegociacionFlow = require('../Flows/reNegociarPrecio/reNegociacionFlow');
    ...
    async handleMessage(userId, message, sock, messageType) {
        ...
        if (flow && flow.flowName) {
            switch (flow.flowName) {
                ...
                case 'RENEGOCIACION':
                    await reNegociacionFlow.Handle(userId, message, flow.currentStep, sock, messageType);
                    break;
